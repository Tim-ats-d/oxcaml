...
All files should be listed on queries: (except `both.ml`)
  ...
  "$TESTCASE_ROOT/sig.mli"
  "$TESTCASE_ROOT/test.mli"
<<<<<<< HEAD:external/merlin/tests/test-dirs/occurrences/project-wide/union.t
||||||| parent of 3bbf3ebbe4 (Add a LRU cache to limit indexing memory usage.):tests/test-dirs/occurrences/project-wide/union.t
...
An error is expected, a pointer references an index file that is considered outdated because we rebuilt it:
  ...
  Hint: try to rebuild indexes with dune build @ocaml-index.
  [1]
>>>>>>> 3bbf3ebbe4 (Add a LRU cache to limit indexing memory usage.):tests/test-dirs/occurrences/project-wide/union.t
